...
The root [usa-verified.m3u](usa-verified.m3u) is the repository's original bulk playlist (4,021 entries uploaded to `main`), filtered by the verification run to entries that are still direct HTTPS HLS **and** returned a media segment; per-entry `policy_format` flags in the verification report show which survivors also pass host review.
